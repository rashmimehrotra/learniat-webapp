...
else
{
    $avg_score = $sum_of_scores/$scores_count;    
			...
			public function UpdateRecordedQuestion($question_id=null, $question_title=null,$question_options=null,$is_ans=null,$col_no=null,$seqno=null)
				{
					try
						{
							/*$get_question = "select question_id from questions where question_id = '$question_id'";
							$got_question = SunDataBaseManager::getSingleton()->QueryDB($get_question);
							$question = mysql_fetch_assoc($got_question);
							$question_id = $question['question_id'];*/
                            ...
                            if (!empty($question_options))
							{
                                ...
                                $seqq1 = array();
                                $colnooo1 = array();
								$count2 = substr_count($question_options, ';;;');
								if ($count2>=1)
								{
									$metooremoved = explode(";;;",$question_options);
                                ...
                                for($p = 0;$p < count($metooremoved);$p++)
                                {
                                    ...
                                    if(!empty($suggest_id))
                                    {
                                        $change_state = "UPDATE  suggestion_options SET  suggestion_state =  '$state' WHERE  suggestion_id ='$suggest_id'";
										// echo "sql:".$change_state;
                                        $change = SunDataBaseManager::getSingleton()->QueryDB($change_state);
                                        if($state == '27' || $state == '28')
                                        {
                                        	$get_class = "select session_id,topic_id,student_id from suggestion_options inner join category on suggestion_options.cat_id=category.category_id where suggestion_id='$suggest_id'";
                                        	$class_info = SunDataBaseManager::getSingleton()->QueryDB($get_class);
                                        	$class = mysql_fetch_assoc($class_info);
                                            ...
                                            //grasp index
                                            //check if row exists 
                                            $get_exists_id = "SELECT auto_id,subtotal_of_count,subtotal_of_score FROM student_index WHERE class_session_id='$session_id' and topic_id = '$topic_id' and student_id='$student_id' and transaction_id=35";
                                            $exists_id_get = SunDataBaseManager::getSingleton()->QueryDB($get_exists_id);
                                            $exists_rows = SunDataBaseManager::getSingleton()->getnoOfrows($exists_id_get);
                                            if($exists_rows > 0)
                                            {
                                            ...
                                            {
                                                //insert if no
                                                $save_device = "insert into student_index(class_session_id,topic_id,transaction_id,student_id,subtotal_of_count,subtotal_of_score) values ('$session_id','$topic_id','35','$student_id','1','-1')";
                                				$save = SunDataBaseManager::getSingleton()->QueryDB($save_device);
                                			}
                                		}
                                        if($state == '30' || $state == '31') //selected correct or wrong
                                        {
                                            $isanswer = 0;
                                            if($state == '30')
                                            {
                                            ...
                                            $count3++; 
                                   			//grasp index
											//check if row exists 
                                            $get_exists_id = "SELECT auto_id,subtotal_of_count,subtotal_of_score FROM student_index WHERE class_session_id='$session_id' and topic_id = '$topic_id' and student_id='$student_id' and transaction_id=34";
                                            $exists_id_get = SunDataBaseManager::getSingleton()->QueryDB($get_exists_id);
                                            $exists_rows = SunDataBaseManager::getSingleton()->getnoOfrows($exists_id_get);
                                            if($exists_rows > 0)
                                            {
                                            ...
                                            {
                                            	//insert if no
                                 				$save_device = "insert into student_index(class_session_id,topic_id,transaction_id,student_id,subtotal_of_count,subtotal_of_score) values ('$session_id','$topic_id','34','$student_id','1','1')";
					                            $save = SunDataBaseManager::getSingleton()->QueryDB($save_device);
					                        }
					                    }
                                        if($state == '29' && (!(empty($col_no) && $col_no != '0'))) //selected correct or wrong
                                        {
                                        	$isanswer = null;
                                            $get_class = "select suggestion_txt,session_id,topic_id,student_id from suggestion_options inner join category on suggestion_options.cat_id=category.category_id where suggestion_id='$suggest_id'";
                                        	$class_info = SunDataBaseManager::getSingleton()->QueryDB($get_class);
...
//4 June 2014
		...
		public function FetchQuestion($question_log_id=null)
				{
					try
						{
							$retval = "<Root><SunStone><Action>";
					...
					try
						{
									...
									$log_query_transition = "insert into suggestion_options(suggestion_txt,cat_id, student_id, session_id, suggestion_state) values('$suggestionText','$category_id','$student_id','$session_id',0)";
...
$logged_query_transition = SunDataBaseManager::getSingleton()->QueryDB($log_query_transition);
            	...
            	try
            	{
            		...
            		if($input_category!=null)
            		{
            			...
            			if($topic_id!=null)
            			{
            				...
            				$select_category="SELECT category_title,category_id,popularity,topic_name
            				FROM category
            				INNER JOIN topic
            				WHERE category_title LIKE '$input_category%'
            				AND category.topic_id=topic.topic_id
            			...
            			while ($x<$i)
            			{
            				...
            				$arr[SMC::$CATEGORYLIST][SMC::$CATEGORY][$x][SMC::$TOPICNAME]=$tpc_name[$x];
            				$x++;
            			}
            			$i1=$i;
            			$i=0;
            			...
            			if($i1<100)
            			{
            				$select_category="select category_title,category_id,popularity,topic_name from category INNER JOIN topic where category_title like '$input_category%' and category.topic_id=topic.topic_id";
            				$category_query=SunDataBaseManager::getSingleton()->QueryDB($select_category);
            				$j=0;
            				while ($row=mysql_fetch_array($category_query))
            				{
            			...
            			if($i1<100)
            			{
            				$select_category="select category_title,category_id,popularity,topic_name from category INNER JOIN topic where cat_descr like '%$input_category%' and category.topic_id=topic.topic_id";
            				$category_query=SunDataBaseManager::getSingleton()->QueryDB($select_category);
            				$j=0;
            				while ($row=mysql_fetch_array($category_query))
            				{
            			...
            			while ($x<$i)
            			{
            				...
            				$arr[SMC::$CATEGORYLIST][SMC::$CATEGORY][$j+$x][SMC::$TOPICNAME]=$tpc_name[$x];
            				$x++;
            			}
            			/*$n=0;
            			while ($n<$i1) {
			...
			/*public function ($_id=null, $_=null)
				...
				public function GetIndex($index_type=null,$student_id=null,$class_id=null,$session_id=null,$topic_id=null)
				{
					try
					{
						$arr[SMC::$STATUS]="Success";
						...
						if(strcasecmp($class_id, 'Agg_Lev=0') && strcasecmp($class_id, 'Agg_Lev=1'))
						{
							...
							else if (!strcasecmp($session_id, 'Agg_Lev=0'))
							{
								...
								if(strcasecmp($topic_id, 'Agg_Lev=0') && strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$ses1="select class_session_id from stud_topic_time where topic_id='$topic_id' and student_id='$student_id'";
					...
					try
					{
						...
						if(is_array($suggestion_ids) && is_array($suggestion_states))
						{
							...
							while ($i<((count($suggestion_ids)<count($suggestion_states)) ? count($suggestion_ids) : count($suggestion_states)))
							{
								...
								if($old_state_id==30 && $sugg_state==28)
								{
						...
						else if(!(is_array($suggestion_ids)) || !(is_array($suggestion_states)))
						{
							...
							if(!(is_array($suggestion_ids)) && is_array($suggestion_states))
							{
								...
								if($old_state_id==30 && $sugg_state1==28)
								{
							...
							else if(is_array($suggestion_ids) && !(is_array($suggestion_states)))
							{
								...
								if($old_state_id==30 && $suggestion_states==28)
								{
				...
				public function SelectCategory($category_id=null,$user_id=null,$device_id=null,$uuid=null)
				{
					...
					{
						$arr[SMC::$STATUS]="Success";
						$select_elems="select element_id,element_text from elements where cat_id='$category_id'";
