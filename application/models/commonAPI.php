	...
	class commonAPI
		{
			...
			public function Login($username = null,$password=null,$app_version = null,$user_id=null,$uuid=null,$app_id=null,$latitude=null,$longitude=null)
				{
					try
						{
                                        ...
                                        if(($exists == 3 && $app_id==1) || ($exists == 4 && $app_id==2))//teacher cant login into student and vice versa
                                        {
                                        	$user_state_s="select user_id,user_state from tbl_auth where user_name = '$username'";
                                        	$state_q=SunDataBaseManager::getSingleton()->QueryDB($user_state_s);
                                        	$state_f=mysql_fetch_assoc($state_q);
                                        	$user_state=$state_f['user_state'];
                                        	$userid=$state_f['user_id'];
                                        	if(($user_state==1)||($user_state==7)||($user_state==9)||($user_state==10)||($user_state==11))
                                        	{
                                        		$uuid_check="select connected_device_id from tbl_auth where user_id='$userid'";
                                        		$uuid_q=SunDataBaseManager::getSingleton()->QueryDB($uuid_check);
                                        		$uuid_f=mysql_fetch_assoc($uuid_q);
                                        		$uuid_v=$uuid_f['connected_device_id'];
                                        		if($uuid!=$uuid_v)
                                        		{
                                        			$error_msg="select error_code,error_message from error_messages where error_id=5";
                                        			$error_msg_q=SunDataBaseManager::getSingleton()->QueryDB($error_msg);
                                        			$msg_f=mysql_fetch_assoc($error_msg_q);
                                        			$msg_v=$msg_f['error_message'];
                                        			$code_v=$msg_f['error_code'];
                                        			$arr[SMC::$ERRORCODE]=$code_v;
                                        			$arr[SMC::$ERRORMESSAGE]=$msg_v;
                                        		}
                                        	}
															...
															if($count > 0)
																{
																	...
																	if($count > 0)
																		{
																			$save_device = "insert into devices(user_id,UUID,latitude,longitude,password,app_id,app_version) values('$uid','$uuid','$latitude','$longitude','$password','$app_id',$app_version)";
			...
			public function GetMyInfo($user_id=null,$uuid=null)
				{
					try
						{
			...
			public function Logout($user_id=null,$uuid=null)
				{
					try
						{
			...
			public function GetMyCurrentSession($user_id=null,$uuid=null)
				{
					try
						{
			...
			public function GetMyNextSession($user_id=null,$uuid=null)
				{
					try
						{
			...
			public function GetMyTodaysSessions($user_id=null,$uuid=null)
				{
					try
						{	
                            //if end time<now(), end it
			...
			public function UpdateSessionState($session_id=null,$status_id=null,$user_id=null,$uuid=null)
				{
					try
						{
							...
							if ($status_id == "1" or $status_id == "5" or $status_id == "6")
											...
											if ($status_id == "5")
												{
                                                    ...
                                                    //close questions
                                                    $getstudents = "select question_log_id from question_log where class_session_id = '$session_id' and end_time is null";                                                    
                									$s_state = SunDataBaseManager::getSingleton()->QueryDB($getstudents);
                									while($cis_state = mysql_fetch_assoc($s_state))
                                                    {      
                                                        $question_log_id = $cis_state['question_log_id']; 
								...
								if($status_id == 5)
								{
									//Check if any query is in state 16/18, yes: put in state 17 and move corresponding volunteer states from 19 to 6 and 20 to 24 if any
									$query_check="select query_id,state from student_query where class_session_id='$session_id' and (state=16 or state=18)";
									$query_q=SunDataBaseManager::getSingleton()->QueryDB($query_check);
									$query_c=SunDataBaseManager::getSingleton()->getnoOfrows($query_q);
									if($query_c>0)
									{
										while ($query_f=mysql_fetch_array($query_q))
										{
											$query_id2=$query_f['query_id'];
											$qstate=$query_f['state'];
											$update_stud_query="update student_query set state=17 where query_id='$query_id2'";
											$update_stud_query_q=SunDataBaseManager::getSingleton()->QueryDB($update_stud_query);
											$select_curr_time="select current_timestamp";
											$curr_time_query=SunDataBaseManager::getSingleton()->QueryDB($select_curr_time);
											$curr_time_fetch=mysql_fetch_assoc($curr_time_query);
											$curr_time=$curr_time_fetch['current_timestamp'];
											$ins_trans="insert into state_transitions(entity_type_id,entity_id,from_state,to_state,transition_time) values(5,'$query_id2','$qstate',17,'$curr_time')";
											$ins_trans_q=SunDataBaseManager::getSingleton()->QueryDB($ins_trans);
											$volunteer_s="select volunteer_id,state from query_volunteer where query_id='$query_id2' and (state=19 or state=20)";
											$volunteer_q=SunDataBaseManager::getSingleton()->QueryDB($volunteer_s);
											$volunteer_c=SunDataBaseManager::getSingleton()->getnoOfrows($volunteer_q);
											if($volunteer_c>0)
											{
												while ($volunteer_f=mysql_fetch_array($volunteer_q))
												{
													$volunteer_id=$volunteer_f['volunteer_id'];
													$vstate=$volunteer_f['state'];
													if($vstate==19)
													{
														$new_state=6;
														$update_v="update query_volunteer set state=6 where volunteer_id='$volunteer_id'";
														$update_v_q=SunDataBaseManager::getSingleton()->QueryDB($update_v);
													}
													else if($vstate==20)
													{
														$new_state=24;
														$update_v="update query_volunteer set state=24 where volunteer_id='$volunteer_id'";
														$update_v_q=SunDataBaseManager::getSingleton()->QueryDB($update_v);
													}
													$select_curr_time="select current_timestamp";
													$curr_time_query=SunDataBaseManager::getSingleton()->QueryDB($select_curr_time);
													$curr_time_fetch=mysql_fetch_assoc($curr_time_query);
													$curr_time=$curr_time_fetch['current_timestamp'];
													$ins_trans="insert into state_transitions(entity_type_id,entity_id,from_state,to_state,transition_time) values(6,'$volunteer_id','$vstate','$new_state','$curr_time')";
													$ins_trans_q=SunDataBaseManager::getSingleton()->QueryDB($ins_trans);
												}
											}
										}
									...
									//Updating gi, pi for this class_id in classes
									...
									$select_topics2="SELECT DISTINCT topic_log.topic_id,parent_topic_id
									FROM topic_log
									INNER JOIN topic
									on topic.topic_id=topic_log.topic_id
									WHERE class_session_id ='$session_id'";
									...
									{
										$parent_topic_id=$tpc_row['parent_topic_id'];
										...
										$tpc_gi_num2=$tpc_gi_num;
										$tpc_gi_den2=$tpc_gi_den;
										$tpc_pi2=$tpc_pi;
										...
										{
											$select_pold_index3="select topic.gi_num,topic.gi_den,topic.pi from topic where topic_id='$parent_topic_id'";
											$pold_index_query3=SunDataBaseManager::getSingleton()->QueryDB($select_pold_index3);
											$pold_index_fetch3=mysql_fetch_assoc($pold_index_query3);
											$pold_gi_num3=$pold_index_fetch3['gi_num'];
											$pold_gi_den3=$pold_index_fetch3['gi_den'];
											$pold_pi3=$pold_index_fetch3['pi'];
											$pold_gi_num3+=$tpc_gi_num2;
											$pold_gi_den3+=$tpc_gi_den2;
											$pold_pi3+=$tpc_pi2;
											$update_ptpc_index="update topic set gi_num='$pold_gi_num3',gi_den='$pold_gi_den3',pi='$pold_pi3' where topic_id='$parent_topic_id'";
											$ptpc_index_query=SunDataBaseManager::getSingleton()->QueryDB($update_ptpc_index);
										}
									}

									//This part is commented because student_class_map is to be removed and new code is added for updating student GI and PI on class level after this commented part
									...
									$sel_class="select class_id from class_sessions where class_session_id='$session_id'";
									$c_q=SunDataBaseManager::getSingleton()->QueryDB($sel_class);
									$c_f=mysql_fetch_assoc($c_q);
									$class_id=$c_f['class_id'];
									$sel_stud="select student_id,student_gi_num,student_gi_den,student_pi from stud_session_time where class_session_id='$session_id'";
									$stud_q=SunDataBaseManager::getSingleton()->QueryDB($sel_stud);
									while ($stud_f=mysql_fetch_array($stud_q))
									{
										$student=$stud_f['student_id'];
										$student_gi_num=$stud_f['student_gi_num'];
										$student_gi_den=$stud_f['student_gi_den'];
										$student_pi=$stud_f['student_pi'];
										$sel_max="select attendance_id,gi_class_num,gi_class_den,pi_class,attendance_id from stud_session_time inner join class_sessions on class_sessions.class_session_id=stud_session_time.class_session_id where student_id='$student' and class_id='$class_id' order by gi_class_den desc limit 1";
										$sel_max_q=SunDataBaseManager::getSingleton()->QueryDB($sel_max);
										$max_f=mysql_fetch_assoc($sel_max_q);
										$att_id=$max_f['attendance_id'];
										$max_gi_num=$max_f['gi_class_num'];
										$max_gi_den=$max_f['gi_class_den'];
										$max_pi=$max_f['pi_class'];
										$max_gi_num+=$student_gi_num;
										$max_gi_den+=$student_gi_den;
										$max_pi+=$student_pi;
										$update_gipi_max="update stud_session_time set gi_class_num='$max_gi_num',gi_class_den='$max_gi_den',pi_class='$max_pi' where attendance_id='$att_id'";
										$max_gipi_q=SunDataBaseManager::getSingleton()->QueryDB($update_gipi_max);
									}
									//*******EMAIL config********
										$mail = new PHPMailer();
        								$mail->IsSMTP();
        								...
        								$mail->SMTPSecure = "tls";
        								$mail->IsHTML(true);
        								set_time_limit(100000); // sets php execution time to 100000 seconds.
        							//*******END OF EMAIL config********
									...
									// This commented part is for sending emails to those who registered but didn't attend. (This is commented for testing purpose)
									...
									while ($row=mysql_fetch_array($students_query))
									{
										...
										if($number_of_sessions==0) // Last session of the day
										{
											...
											/*To send emails to all parents irrespective of the fact that the students did or didn't attend a single session in the day: (Commented for time being for testing purpose, should be uncommented actually)
			...
			public function ConfigureGrid($room_id=null, $rows=null, $columns=null, $seats_removed=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function GetStudentsSessionInfo($session_id=null,$user_id=null,$uuid=null)
				{
					try
						{
...
//return rows for students
			...
			public function RetrieveStudentQuery($query_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function StudentSeatAssignment($session_id=null,$seat_id=null,$student_id=null,$status_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function RetrieveGridDesign($room_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function RetrieveSeatAssignments($session_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function ClassSessionSummary($session_id=null,$user_id=null,$uuid=null)
				{
					try
						{
							...
							$question_count = SunDataBaseManager::getSingleton()->getnoOfrows($questions_configured);                               
															...
															$validate = SunDataBaseManager::getSingleton()->QueryDB($validate_password);
																	...
																	$school_id = $user['school_id'];
							/********************* changes ************************/
							// To find out the total number of questions asked in this session
							$tot_questions="SELECT COUNT(DISTINCT question_id) as total_questions
							FROM question_log
							WHERE class_session_id='$session_id'";
							$tot_questions_q=SunDataBaseManager::getSingleton()->QueryDB($tot_questions);
							$tot_questions_f=mysql_fetch_assoc($tot_questions_q);
							$total_questions=$tot_questions_f['total_questions'];
							//To find out total numbers of answers received for all questions
							$tot_answers="SELECT sum(total_answers) as total_ans
							FROM
							(SELECT COUNT(DISTINCT student_id) as total_answers
							FROM assessment_answers
							INNER JOIN question_log
							on assessment_answers.question_log_id=question_log.question_log_id
							INNER JOIN questions
							on questions.question_id=question_log.question_id
							WHERE assessment_answers.class_session_id='$session_id'
							GROUP by questions.question_id) as tot_answers";
							$tot_answers_q=SunDataBaseManager::getSingleton()->QueryDB($tot_answers);
							$tot_answers_f=mysql_fetch_assoc($tot_answers_q);
							$total_answers2=$tot_answers_f['total_ans'];
							if($total_answers2==null)
							{
								$total_answers2=0;
							}
							//To find out total numbers of sub topics completed
							$tot_sub_topics = "SELECT COUNT(sub_topics) AS subtopics
							FROM
							(SELECT DISTINCT topic.topic_id as sub_topics
							FROM topic_log
							INNER JOIN topic
							ON topic.topic_id=topic_log.topic_id
							WHERE parent_topic_id IS NOT null
							AND class_session_id='$session_id') AS topics";
							$tot_sub_topics_q=SunDataBaseManager::getSingleton()->QueryDB($tot_sub_topics);
							$tot_sub_topics_f=mysql_fetch_assoc($tot_sub_topics_q);
							$total_subtopics=$tot_sub_topics_f['subtopics'];
							//To find out total numbers of parent topics completed
							$tot_par_topics = "SELECT COUNT(par_topics) AS parenttopics
							FROM
							(SELECT DISTINCT topic.topic_id as par_topics
							FROM topic_log
							INNER JOIN topic
							ON topic.topic_id=topic_log.topic_id
							WHERE parent_topic_id IS null
							AND class_session_id='$session_id') AS topics";
							$tot_par_topics_q=SunDataBaseManager::getSingleton()->QueryDB($tot_par_topics);
							$tot_par_topics_f=mysql_fetch_assoc($tot_par_topics_q);
							$total_partopics=$tot_par_topics_f['parenttopics'];
							//Total sub topics configured
							$tot_sub_tpc_conf="SELECT COUNT(lesson_plan.topic_id) as tot_conf
							FROM lesson_plan
							INNER JOIN class_sessions
							ON class_sessions.class_id=lesson_plan.class_id
							INNER JOIN topic
							ON lesson_plan.topic_id=topic.topic_id
							WHERE class_session_id='$session_id'
							AND parent_topic_id IS NOT null";
							$tot_sub_tpc_conf_q=SunDataBaseManager::getSingleton()->QueryDB($tot_sub_tpc_conf);
							$tot_sub_tpc_conf_f=mysql_fetch_assoc($tot_sub_tpc_conf_q);
							$tot_subtopics_conf=$tot_sub_tpc_conf_f['tot_conf'];
							//Total main topics configured
							$tot_main_tpc_conf="SELECT COUNT(lesson_plan.topic_id) as tot_main_conf
							FROM lesson_plan
							INNER JOIN class_sessions
							ON class_sessions.class_id=lesson_plan.class_id
							INNER JOIN topic
							...
							$tot_main_tpc_conf_q=SunDataBaseManager::getSingleton()->QueryDB($tot_main_tpc_conf);
							$tot_main_tpc_conf_f=mysql_fetch_assoc($tot_main_tpc_conf_q);
							$tot_maintopics_conf=$tot_main_tpc_conf_f['tot_main_conf'];
							/********************* end of changes ************************/
							$arr[SMC::$STATUS] = "Success";
							$arr[SMC::$SEATSCONFIGURED] = $configured_count;
							$arr[SMC::$STUDENTSREGISTERED] = $registered_count;
							...
							$arr[SMC::$TOTALQUESTIONSCOMPLETED] = $total_questions;
							$arr[SMC::$TOTALANSWERSRECEIVED] = $total_answers2;
							$arr[SMC::$TOTALSUBTOPICSCOMPLETED] = $total_subtopics;
							$arr[SMC::$TOTALMAINTOPICSCOMPLETED] = $total_partopics;
							$arr[SMC::$TOTALSUBTOPICSCONFIGURED] = $tot_subtopics_conf;
							$arr[SMC::$TOTALMAINTOPICSCONFIGURED] = $tot_maintopics_conf;
                            $retval = $this->mXMLManager->createXML($arr);
							return $retval;
						}
					catch(Exception $e)
			...
			public function ResetSeatAssignment($session_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function ExtendSessionTime($session_id=null, $extended_time=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function GetAllNodes($class_id=null, $subject_id=null,$main_topic_id=null,$type=null,$user_id=null,$uuid=null)
				{
					try
						{
...
{											
	$topic_gi="select gi_num,gi_den,pi from lesson_plan where topic_id='$topic_id' and class_id='$class_id'";
	$gi_q=SunDataBaseManager::getSingleton()->QueryDB($topic_gi);
	$gi_f=mysql_fetch_assoc($gi_q);
	$gi_num=$gi_f['gi_num'];
	$gi_den=$gi_f['gi_den'];
	$pi=$gi_f['pi'];
	$gi_achieved=$gi_num/$gi_den;
	$gipi_thres="select threshold_value,threshold_code from threshold_values inner join subjects on subjects.subject_id='$subject_id' where threshold_values.school_id=subjects.school_id";
	$thres_q=SunDataBaseManager::getSingleton()->QueryDB($gipi_thres);
	$att_thres=0;
	$gi_thres=0;
	$pi_thres=0;
	$question_thres=0;
	while ($thres_f=mysql_fetch_array($thres_q))
	{
		$thres_code=$thres_f['threshold_code'];
		if($thres_code==1)
		{
			$att_thres==$thres_f['threshold_value'];
		}
		if($thres_code==2)
		{
			$gi_thres==$thres_f['threshold_value'];
		}
		if($thres_code==3)
		{
			$pi_thres==$thres_f['threshold_value'];
		}
		if($thres_code==2)
		{
			$question_thres==$thres_f['threshold_value'];
		}
	}
	$att_ach_s="SELECT (sum(stud_time)/count(student_id))/3600 as att_achieved FROM stud_topic_time inner join class_sessions on class_sessions.class_session_id=stud_topic_time.class_session_id where topic_id='$topic_id' and class_id='$class_id'";
	$att_q=SunDataBaseManager::getSingleton()->QueryDB($att_ach_s);
	$att_f=mysql_fetch_assoc($att_q);
	$att_ach=$att_f['att_achieved'];
	$unique_questions_s="SELECT COUNT(DISTINCT question_id) as unique_questions FROM questions inner join lesson_plan on lesson_plan.topic_id=questions.topic_id where lesson_plan.topic_id='$topic_id' and class_id='$class_id'";
	$unique_q=SunDataBaseManager::getSingleton()->QueryDB($unique_questions);
	$unique_f=mysql_fetch_assoc($unique_q);
	$unique_questions=$unique_f['unique_questions'];
	if($gi_thres==0)
	{
		$gi_complete=0;
	}
	...
	{
		$gi_complete=$gi_achieved/$gi_thres;
		if($gi_complete>1)
		{
			$gi_complete=1;
		}
	...
	{
		$att_complete=$att_ach/$att_thres;
	}
	if($question_thres==0)
	{
		$questions_comp=0;
	...
	{
		$questions_complete=$unique_questions/$question_thres;
	}
	$sub_topic_percentage_started=($gi_complete*$pi_complete*$att_complete*$questions_complete*100)."%";
	//$sub_topic_percentage_started = (($initiated_topic_count/$tagged_sub_topic_count)*100)."%";
}
else
{
	$sub_topic_percentage_started = 0;
}
...
else
{
    $avg_score = $sum_of_scores/$scores_count;    
			...
			public function SetCurrentTopic($session_id=null,$topic_id=null,$student_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function StopTopic($session_id=null,$topic_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function OpenQueryForVoting($query_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function BroadcastQuestion($session_id=null, $question_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function CloseQuestionResponse($question_log_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function EndQuestionInstance($question_log_id=null,$topic_id=null, $student_id=null,$user_id=null,$uuid=null)
				{
					try
						{
							$change_state = "update question_log set active_question = '0', end_time = NOW() where question_log_id = '$question_log_id'";
			...
			public function ApproveVolunteer($volunteer_id=null, $stoppedflag=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function RetrieveVolunteerVotes($volunteer_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function RecordLessonPlan($class_id=null, $teacher_id=null, $topic_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function RecordModelAnswer($answer_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function UploadTeacherScribble($image_path=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function RecordQuestion($session_id=null, $type_id=null, $topic_id=null, $teacher_id=null, $scribble_id=null, $question_title=null,$user_id=null,$uuid=null)
				{
					try
						{
				...
				//OLD UpdateRecordedQuestion() API:
			...
			/*public function UpdateRecordedQuestion($question_id=null, $question_title=null,$element_id=null,$is_ans=null,$col_no=null,$seqno=null)
				...
				public function UpdateRecordedQuestion($question_id=null, $question_title=null,$element_id=null,$is_ans=null,$col_no=null,$seqno=null,$user_id=null,$uuid=null)
				{
					try
					{
							$update = "update questions set question_name = '$question_title' where question_id = '$question_id'";
			...
			public function RetrieveStudentAnswer($assessment_answer_id=null,$user_id=null,$uuid=null)
				{
					try
						{
							...
							if($count > 0)
								{
									...
									if ($type == "Multiple Response")
										{									  
											...
											while($option = mysql_fetch_assoc($option_list))
        										...
        										while($option1 = mysql_fetch_assoc($option_list1))
                                                    ...
                                                    {
                                                        $arr[SMC::$OPTIONCONTAINER][SMC::$OPTION][$i][SMC::$ISANSWER] = 1;
                                                        break;
                                                    }
                                                    else
                                                    {
                                                        $arr[SMC::$OPTIONCONTAINER][SMC::$OPTION][$i][SMC::$ISANSWER] = 0;
                                                    }
  
		                                            }
													$arr[SMC::$OPTIONCONTAINER][SMC::$OPTION][$i][SMC::$OPTIONTEXT] = $option['option_text'];
										...
										//	$option = mysql_fetch_assoc($option_list);
                                            ...
                                            $arr[SMC::$OPTIONCONTAINER][SMC::$OPTION][$i][SMC::$OPTIONTEXT] = $combo;
									...
									else if ($type == "Match Columns")
										{
											...
											while($option = mysql_fetch_assoc($option_list))
												{
													$arr[SMC::$OPTIONCONTAINER][SMC::$OPTION][$i][SMC::$OPTIONTEXT] = $option['option_text'];
													$arr[SMC::$OPTIONCONTAINER][SMC::$OPTION][$i][SMC::$COLUMN] = $option['mtc_column'];
													$arr[SMC::$OPTIONCONTAINER][SMC::$OPTION][$i][SMC::$SEQUENCE] = $option['mtc_sequence'];
                                                    $arr[SMC::$OPTIONCONTAINER][SMC::$OPTION][$i][SMC::$OLDSEQ] = $option['old_sequence'];
													$i++;
												}
										}
								}
			...
			public function SendFeedback($answer_id=null, $user_id=null, $url=null, $rating=null, $text_rating=null, $badge_id=null,$student_id=null, $model=null,$uuid=null)
				{
					try
						{
...
//4 June 2014
			public function StopVolunteering($volunteer_id=null, $thumbs_up=null, $thumbs_down=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function SaveVolunteerFeedback($volunteer_id=null, $rating=null, $badge_id=null,$user_id=null,$uuid=null)
				{
					try
						{
							$state = "5";
			...
			public function RespondToQuery($query_id=null, $response_text=null, $badge_id=null, $dismiss=null, $student_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function SaveSelectedQueries($query_id=null, $volunteer=null,$user_id=null,$uuid=null)
				{
					try
						{
							$state = "18";
			...
			public function GetVolunteerList($query_id=null,$user_id=null,$uuid=null)
				{
					try
						{
    			...
    			public function UpdateUserState($user_id=null,$status_id=null,$session_id=null,$uuid=null)
    				{
    					try
    						{
    						    if ($status_id)
                ...
                public function GetSessionInfo($session_id=null,$user_id=null,$uuid=null)
				{
					try
						{
							$session_state = "select session_state,class_sessions.teacher_id,starts_on,ends_on,timezone from class_sessions inner join classes on class_sessions.class_id=classes.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id inner join schools on academic_terms.school_id=schools.school_id where class_session_id = '$session_id'";
							$retrieve_session_state = SunDataBaseManager::getSingleton()->QueryDB($session_state);
  							...
  							if ($count > 0)
								{
									...
									while($teacher= mysql_fetch_assoc($retrieve_session_state))
										{
											$starts_on=$teacher['starts_on'];
											$ends_on=$teacher['ends_on'];
											$timezone=$teacher['timezone'];
											$start_time=substr($starts_on, 0,11);
											$hours=0;
											$minutes=0;
											$seconds=substr($starts_on, 17,2)+substr($timezone, 6,2);
											if($seconds>=60)
											{
												$seconds-=60;
												$minutes++;
											...
											$minutes+=substr($starts_on, 14,2)+substr($timezone, 3,2);
											if($minutes>=60)
											{
												$minutes-=60;
												$hours++;
											...
											$hours+=substr($starts_on, 11,2)+substr($timezone, 0,2);
											if($hours>=24)
											{
												$hours-=24;
											}
											...
											$start_time=$start_time.$hours.':'.$minutes.':'.$seconds;
											$end_time=substr($ends_on, 0,11);
											$hours=0;
											$minutes=0;
											$seconds=substr($ends_on, 17,2)+substr($timezone, 6,2);
											if($seconds>=60)
											{
												$seconds-=60;
												$minutes++;
											...
											$minutes+=substr($ends_on, 14,2)+substr($timezone, 3,2);
											if($minutes>=60)
											{
												$minutes-=60;
												$hours++;
											...
											$hours+=substr($ends_on, 11,2)+substr($timezone, 0,2);
											if($hours>=24)
											{
												$hours-=24;
											}
											...
											$end_time=$end_time.$hours.':'.$minutes.':'.$seconds;
										  	$arr[SMC::$TEACHERID] = $teacher['teacher_id'];
										  	$arr[SMC::$SESSIONSTATE] = $teacher['session_state'];
										  	$arr[SMC::$STARTTIME] = $start_time;
										  	$arr[SMC::$ENDTIME] = $end_time;
		...
		public function FetchQuestion($question_id=null,$question_log_id=null,$user_id=null,$uuid=null)
				{
					try
						{
							$retval = "<Root><SunStone><Action>";
			...
			public function GetFeedback($assessment_answer_id=null,$user_id=null,$uuid=null)
				{
					try
						{
			...
			public function RetrieveBadges($user_id=null,$uuid=null)
				{
					try
						{
 			...
 			public function GetThisStudentSessions($user_id=null,$uuid=null)
				{
					try
						{
...
where sclassmap.student_id ='$user_id' and date(term.starts_on) = curdate()";
			...
			public function SendSignupInfo($deviceId=null,$institutionName=null,$email=null,$contactPerson=null,$phoneNo=null,$ipadFlag=null, $address=null,$city=null,$longitude=null, $latitude=null,$user_id=null,$uuid=null)
				{
					try
						{
...
EOF;
 			...
 			public function GetParticipationIndex($session_id=null,$student_id=null,$user_id=null,$uuid=null)
				{
					try
						{
						  if($student_id == null)
 			...
 			public function GetGraspIndex($session_id=null,$student_id=null,$topic_id=null,$user_id=null,$uuid=null)
				{
					try
						{
						  if($student_id == null)
			...
			public function GetMyState($user_id=null,$uuid=null)
				{
					try
						{
    			...
    			public function GetQueryResponse($query_id=null,$user_id=null,$uuid=null)
    				{
    					try
    						{
    						  if($query_id != null)
        			...
        			public function SaveMeTooVotes($query_id=null, $student_id=null)
        				{
               			...
               			public function FetchSRQ($session_id=null,$user_id=null,$uuid=null)
            				{
            					try
            						{
            						  if($session_id != null)
    			...
    			public function VolunteerRegister($student_id=null,$query=null)
    				{
    					try
                			...
                			public function GetVolunteerSelected($session_id=null,$user_id=null,$uuid=null)
            				{
            					try
            						{
            						  if($session_id != null)
        			...
        			public function WithdrawStudentSubmission($AssessmentAnswerId = null,$query_id=null,$user_id=null,$uuid=null)
        				{
        					try
        						{
                 			...
                 			public function RetrieveAggregateDrillDown($question_log_id=null, $seq=null,$user_id=null,$uuid=null)
            				{
            					try
            						{
 ...
 //incorrect
 			...
 			public function GetAllRooms($school_id=null,$user_id=null,$uuid=null)
				{
					try
						{
						    $retval = "<Root><SunStone><Action>";
    			...
    			public function GetModelAnswer($question_log_id=null,$user_id=null,$uuid=null)
    				{
    					try
    						{
   			...
   			public function GetAllModelAnswers($question_log_id=null,$user_id=null,$uuid=null)
    				{
    					try
    						{
    			...
    			public function EndVolunteeringSession($session_id=null,$queryidlist=null,$metoolist= null,$user_id=null,$uuid=null)
    				{
    					try
    						{
                                        $removed = null;
        			...
        			public function GetMaxStudentsRegistered($roomId=null,$user_id=null,$uuid=null)
        				{
        					try
        						{
			...
			public function GetState($entity_type=null, $entity_id=null,$user_id=null,$uuid=null)
				{
					try
						{
						  if($entity_type == "user")
       			...
       			public function GetQuestionDetails($question_id=null,$user_id=null,$uuid=null)
        				{
        					try
        						{
                                    $ques_scores = "SELECT answer_score FROM assessment_answers WHERE question_log_id in (select question_log_id from question_log where question_id='$question_id')";                                                                            
			...
			public function GetAllStudentStates($session_id=null,$user_id=null,$uuid=null)
				{
					try
						{
 			...
 			public function GetAllStudentIndex($session_id=null,$topic_id=null,$user_id=null,$uuid=null)
				{
					try
						{
                            if($topic_id == null)
...
ORDER BY user.first_name";
			...
			public function RecoverFromCrash($session_id=null,$type=null,$user_id=null,$uuid=null)
				{
					try
						{
                                   ...
                                   // echo "quesid:".$QuestionId;
									...
									if ($type == "Multiple Response")
        										...
        										while($option1 = mysql_fetch_assoc($option_list1))
        											{
                                                     ...
                                                     if(trim($option['option_text']) == trim($option_text) && $answer == 1)
                                                    {
                                                        $arr[SMC::$STUDENTS][SMC::$STUDENT][$i][SMC::$OPTIONCONTAINER][SMC::$OPTION][$j][SMC::$ISANSWER] = 1;
                                                        break;
                                                    }
                                                    else
                                                    {
                                                        $arr[SMC::$STUDENTS][SMC::$STUDENT][$i][SMC::$OPTIONCONTAINER][SMC::$OPTION][$j][SMC::$ISANSWER] = 0;
                                                    }
  
		                                            }
													$arr[SMC::$STUDENTS][SMC::$STUDENT][$i][SMC::$OPTIONCONTAINER][SMC::$OPTION][$j][SMC::$OPTIONTEXT] = $option['option_text'];
										...
										//	$option = mysql_fetch_assoc($option_list);
                                            ...
                                            $arr[SMC::$STUDENTS][SMC::$STUDENT][$i][SMC::$OPTIONCONTAINER][SMC::$OPTION][$j][SMC::$OPTIONTEXT] = $combo;
									...
									else if ($type == "Match Columns")
										{
											...
											while($option = mysql_fetch_assoc($option_list))
												{
													$arr[SMC::$STUDENTS][SMC::$STUDENT][$i][SMC::$OPTIONCONTAINER][SMC::$OPTION][$j][SMC::$OPTIONTEXT] = $option['option_text'];
													$arr[SMC::$STUDENTS][SMC::$STUDENT][$i][SMC::$OPTIONCONTAINER][SMC::$OPTION][$j][SMC::$COLUMN] = $option['mtc_column'];
													$arr[SMC::$STUDENTS][SMC::$STUDENT][$i][SMC::$OPTIONCONTAINER][SMC::$OPTION][$j][SMC::$SEQUENCE] = $option['mtc_sequence'];
                                                    $arr[SMC::$STUDENTS][SMC::$STUDENT][$i][SMC::$OPTIONCONTAINER][SMC::$OPTION][$j][SMC::$OLDSEQ] = $option['old_sequence'];
													$j++;
												}
										}
								}
			...
			public function RecordSuggestion($student_id=null,$suggestionText=null,$category_id=null, $session_id=null, $topic_id=null,$uuid=null)
				{
					try
						{
...
$logged_query_transition = SunDataBaseManager::getSingleton()->QueryDB($log_query_transition);
    			...
    			public function ClearQuestion($queryidlist=null,$user_id=null,$uuid=null)
    				{
    					try
    						{
													if (!empty($queryidlist))
            ...
            public function FetchCategory($input_category=null,$topic_id=null,$user_id=null,$uuid=null)
            {
            	try
            	{
            		$arr[SMC::$STATUS]="Success";
			...
			/*public function ($_id=null, $_=null)
				...
				public function GetIndex($index_type=null,$student_id=null,$class_id=null,$session_id=null,$topic_id=null,$user_id=null,$uuid=null)
				{
					try
					{
						$arr[SMC::$STATUS]="Success";
						...
						if($student_id==null)
						{
							$student_id="Agg_Lev=0";
						}
						$school_s="select school_id from tbl_auth where user_id='$user_id'";
						$school_q=SunDataBaseManager::getSingleton()->QueryDB($school_s);
						$school_f=mysql_fetch_assoc($school_q);
						$school_id=$school_f['school_id'];
						...
						if(strcasecmp($class_id, 'Agg_Lev=0') && strcasecmp($class_id, 'Agg_Lev=1'))
						{
									...
									else if (!strcasecmp($student_id, 'Agg_Lev=0'))
									{
										...
										if($index_type==1)
										{
											...
											while ($stud_f1=mysql_fetch_array($stud_q1))
											{
												...
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$STUDENTID]=$stud_v1;
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$GRASPINDEX]=$index_v3;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($stud_f1=mysql_fetch_array($stud_q1))
											{
												...
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$STUDENTID]=$stud_v1;
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$PARTICIPATIONINDEX]=$index_v4;
												$i++;
											}
										}
									}
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										...
										if($index_type==1)
										{
											...
											while ($topic_f1=mysql_fetch_array($topic_q1))
											{
												...
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$TOPICID]=$topic_v1;
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$GRASPINDEX]=$index_v7;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($topic_f1=mysql_fetch_array($topic_q1))
											{
												...
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$TOPICID]=$topic_v1;
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$PARTICIPATIONINDEX]=$index_v8;
												$i++;
											}
										}
									}
										...
										if($index_type==1)
										{
											...
											while ($topic_f2=mysql_fetch_array($topic_q2))
											{
												...
												while($stud_f3=mysql_fetch_array($stud_q3))
												{
													...
													$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$j][SMC::$TOPICID]=$topic_v2;
													$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$j][SMC::$STUDENTID]=$stud_v3;
													$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$j][SMC::$GRASPINDEX]=$index_v9;
													$j++;
												}
											}
										}
										...
										else if($index_type==2)
										{
											...
											while ($topic_f2=mysql_fetch_array($topic_q2))
											{
												...
												while($stud_f3=mysql_fetch_array($stud_q3))
												{
													...
													$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$j][SMC::$TOPICID]=$topic_v2;
													$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$j][SMC::$STUDENTID]=$stud_v3;
													$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$j][SMC::$PARTICIPATIONINDEX]=$index_v10;
													$j++;
												}
											}
										}
									...
									else if (!strcasecmp($student_id, 'Agg_Lev=1'))
									{
										...
										if($index_type==1)
										{
											...
											while ($topic_f3=mysql_fetch_array($topic_q3))
											{
												...
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$TOPICID]=$topic_v3;
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$GRASPINDEX]=$index_v11;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($topic_f3=mysql_fetch_array($topic_q3))
											{
												...
												$index_v12=$num;
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$TOPICID]=$topic_v3;
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$PARTICIPATIONINDEX]=$index_v12;
												$i++;
											}
										}
									}
									...
									else if (!strcasecmp($student_id, 'Agg_Lev=0'))
									{
										...
										if($index_type==1)
										{
											...
											while ($stud_f5=mysql_fetch_array($stud_q5))
											{
												...
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$STUDENTID]=$stud_v5;
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$GRASPINDEX]=$index_v15;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($stud_f5=mysql_fetch_array($stud_q5))
											{
												...
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$STUDENTID]=$stud_v5;
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$PARTICIPATIONINDEX]=$index_v16;
												$i++;
											}
										}
									}
							...
							else if (!strcasecmp($session_id, 'Agg_Lev=0'))
							{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										...
										if($index_type==1)
										{
											...
											while ($ses_f1=mysql_fetch_array($ses_q1))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v1;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v19;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($ses_f1=mysql_fetch_array($ses_q1))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v1;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$PARTICIPATIONINDEX]=$index_v20;
												$i++;
											}
										}
									}
										...
										if($index_type==1)
										{
											...
											while ($ses_f2=mysql_fetch_array($ses_q2))
											{
												...
												while ($stud_f7=mysql_fetch_array($stud_q7))
												{
													...
													$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v2;
													$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$STUDENTID]=$stud_v7;
													$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v21;
													$i++;
												}
											}
										}
										...
										else if($index_type==2)
										{
											...
											while ($ses_f2=mysql_fetch_array($ses_q2))
											{
												...
												while ($stud_f7=mysql_fetch_array($stud_q7))
												{
													...
													$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v2;
													$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$STUDENTID]=$stud_v7;
													$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$PARTICIPATIONINDEX]=$index_v22;
													$i++;
												}
											}
										}
									...
									else if (!strcasecmp($student_id, 'Agg_Lev=1'))
									{
										...
										if($index_type==1)
										{
											...
											while ($ses_f3=mysql_fetch_array($ses_q3))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v3;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v23;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($ses_f3=mysql_fetch_array($ses_q3))
											{
												...
												$index_v24=$num;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v3;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$PARTICIPATIONINDEX]=$index_v24;
												$i++;
											}
										}
									}
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										...
										if($index_type==1)
										{
											...
											while ($stud_f9=mysql_fetch_array($stud_q9))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v4;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$TOPICID]=$topic_v4;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v25;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($stud_f9=mysql_fetch_array($stud_q9))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v4;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$TOPICID]=$topic_v4;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$PARTICIPATIONINDEX]=$index_v26;
												$i++;
											}
										}
									}
									...
									else if (!strcasecmp($student_id, 'Agg_Lev=0'))
									{
										...
										if($index_type==1)
										{
											...
											while ($ses_f5=mysql_fetch_assoc($ses_q5))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v5;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$TOPICID]=$topic_v5;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$STUDENTID]=$stud_v10;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v27;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($ses_f5=mysql_fetch_assoc($ses_q5))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$TOPICID]=$topic_v5;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$STUDENTID]=$stud_v10;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$PARTICIPATIONINDEX]=$index_v28;
												$i++;
											}
										}
									}
									...
									else if (!strcasecmp($student_id, 'Agg_Lev=1'))
									{
										...
										if($index_type==1)
										{
											...
											while ($ses_f6=mysql_fetch_array($ses_q6))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v6;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$TOPICID]=$topic_v6;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v29;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($ses_f6=mysql_fetch_array($ses_q6))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v6;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$TOPICID]=$topic_v6;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$PARTICIPATIONINDEX]=$index_v30;
												$i++;
											}
										}
									}
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										...
										if($index_type==1)
										{
											...
											while ($ses_f7=mysql_fetch_array($ses_q7))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v7;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v31;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($ses_f7=mysql_fetch_array($ses_q7))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v7;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$PARTICIPATIONINDEX]=$index_v32;
												$i++;
											}
										}
									}
									...
									else if (!strcasecmp($student_id, 'Agg_Lev=0'))
									{
										...
										if($index_type==1)
										{
											...
											while ($stud_f11=mysql_fetch_array($stud_q11))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v8;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$STUDENTID]=$stud_v11;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v33;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($stud_f11=mysql_fetch_array($stud_q11))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v8;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$STUDENTID]=$stud_v11;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$PARTICIPATIONINDEX]=$index_v34;
												$i++;
											}
										}
									}
									...
									{
										$ses9="select distinct class_sessions.class_session_id,gi_num,gi_den,pi from class_sessions inner join stud_session_time on stud_session_time.class_session_id=class_sessions.class_session_id where class_id='$class_id'";
										...
										if($index_type==1)
										{
											...
											while ($ses_f9=mysql_fetch_array($ses_q9))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v9;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v35;
												$i++;
											}
										}
										//PI
										else if($index_type==2)
										{
											...
											while ($ses_f9=mysql_fetch_array($ses_q9))
											{
												...
												$index_v36=$ses_f9['pi'];
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$ses_v9;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$PARTICIPATIONINDEX]=$index_v36;
												$i++;
											}
										}
									}
							...
							else if (!strcasecmp($session_id, 'Agg_Lev=1'))
							{
								...
								if(strcasecmp($topic_id, 'Agg_Lev=0') && strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									{
										$stud_s="select distinct student_id from stud_topic_time inner join class_sessions on stud_topic_time.class_session_id=class_sessions.class_session_id where topic_id='$topic_id' and class_id='$class_id'";
										$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
										if($index_type==1)
										{
											$i=0;
											while ($stud_f=mysql_fetch_array($stud_q))
											{
												$stud_v=$stud_f['student_id'];
												$sess_s="select distinct class_sessions.class_session_id,student_gi_num,student_gi_den from stud_topic_time inner join class_sessions on class_sessions.class_session_id=stud_topic_time.class_session_id where student_id='$stud_v' and class_id='$class_id' and topic_id='$topic_id'";
												$sess_q=SunDataBaseManager::getSingleton()->QueryDB($sess_s);
												$num=0;
												$den=0;
												while ($sess_f=mysql_fetch_array($sess_q))
												{
													$num+=$sess_f['student_gi_num'];
													$den+=$ses_f['student_gi_den'];
												}
												if($den==0)
												{
													$index_v=0;
												...
												{
													$index_v=substr((($num*100)/$den),0,strpos((($num*100)/$den),'.')+3);
												}
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$STUDENTID]=$stud_v;
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$GRASPINDEX]=$index_v;
												$i++;
											}
										}
										else if($index_type==2)
										{
											...
											{
												$stud_v=$stud_f['student_id'];
												$sess_s="select distinct class_sessions.class_session_id,student_pi from stud_topic_time inner join class_sessions on class_sessions.class_session_id=stud_topic_time.class_session_id where student_id='$stud_v' and class_id='$class_id' and topic_id='$topic_id'";
												$sess_q=SunDataBaseManager::getSingleton()->QueryDB($sess_s);
												$pi=0;
												while ($sess_f=mysql_fetch_array($sess_q))
												{
													$pi+=$sess_f['student_pi'];
												}
												$index_v=$pi;
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$STUDENTID]=$stud_v;
												$arr[SMC::$STUDENTCONTAINER][SMC::$STUDENT][$i][SMC::$PARTICIPATIONINDEX]=$index_v;
												$i++;
											}
										}
									}
									...
									{
										$topic_s="select gi_num,gi_den,pi from lesson_plan where topic_id='$topic_id' and class_id='$class_id'";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$topic_f=mysql_fetch_assoc($topic_q);
											$num=$topic_f['gi_num'];
											$den=$topic_f['gi_den'];
											if($den==0)
											{
											...
											$arr[SMC::$GRASPINDEX]=$index_v;
										}
										else if($index_type==2)
										{
											$topic_f=mysql_fetch_assoc($topic_q);
											$pi=$topic_f['pi'];
											$arr[SMC::$PARTICIPATIONINDEX]=$index_v;
										}
									}
								}
								//topic_id has aggregation_level=0
								else if (!strcasecmp($topic_id, 'Agg_Lev=0'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$topic_s="select distinct stud_topic_time.topic_id from stud_topic_time inner join lesson_plan on lesson_plan.topic_id=stud_topic_time.topic_id where student_id='$student_id' and class_id='$class_id'";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$i=0;
											while ($topic_f=mysql_fetch_array($topic_q))
											{
												$topic_v=$topic_f['topic_id'];
												$sess_s="select class_session_id,student_gi_num,student_gi_den from stud_topic_time where topic_id='$topic_v' and student_id='$student_id'";
												$sess_q=SunDataBaseManager::getSingleton()->QueryDB($sess_s);
												$num=0;
												$den=0;
												while ($sess_f=mysql_fetch_array($sess_q))
												{
													$sess_v=$sess_f['class_session_id'];
													$num+=$sess_f['student_gi_num'];
													$den+=$sess_f['student_gi_den'];
												}
												if($den==0)
												{
													$index_v=0;
												...
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$TOPICID]=$topic_v;
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$GRASPINDEX]=$index_v;
												$i++;
											}
										}
										else if($index_type==2)
										{
											...
											{
												$topic_v=$topic_f['topic_id'];
												$sess_s="select class_session_id,student_pi from stud_topic_time where topic_id='$topic_v' and student_id='$student_id'";
												$sess_q=SunDataBaseManager::getSingleton()->QueryDB($sess_s);
												$pi=0;
												while ($sess_f=mysql_fetch_array($sess_q))
												{
												...
												$index_v=$pi;
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$TOPICID]=$topic_v;
												$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$PARTICIPATIONINDEX]=$index_v;
												$i++;
											}
										}
									}
									...
									{
										$topic_s="select distinct topic_id from stud_topic_time inner join class_sessions on class_sessions.class_session_id=stud_topic_time.class_session_id where class_id='$class_id'";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$i=0;
											while ($topic_f=mysql_fetch_assoc($topic_q))
											{
												$topic_v=$topic_f['topic_id'];
												$stud_s="select distinct student_id from stud_topic_time where topic_id='$topic_v'";
												$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
												while ($stud_f=mysql_fetch_array($stud_q))
												{
													$stud_v=$stud_f['student_id'];
													$sess_s="select class_session_id,student_gi_num,student_gi_den from stud_topic_time where topic_id='$topic_v' and student_id='$stud_v'";
													$sess_q=SunDataBaseManager::getSingleton()->QueryDB($sess_s);
													$num=0;
													$den=0;
													while ($sess_f=mysql_fetch_assoc($sess_q))
													{
														$sess_v=$sess_f['class_session_id'];
														$num+=$sess_f['student_gi_num'];
														$den+=$sess_f['student_gi_den'];
													...
													$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$TOPICID]=$topic_v;
													$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$STUDENTID]=$stud_v;
													$arr[SMC::$TOPICCONTAINER][SMC::$TOPIC][$i][SMC::$GRASPINDEX]=$index_v;
													$i++;
												}
											}
										...
										else if($index_type==2)
										{
											...
											while ($topic_f=mysql_fetch_assoc($topic_q))
											{
												...
												{
													$stud_v=$stud_f['student_id'];
													$sess_s="select class_session_id,student_pi from stud_topic_time where topic_id='$topic_v' and student_id='$stud_v'";
													$sess_q=SunDataBaseManager::getSingleton()->QueryDB($sess_s);
													$pi=0;
													while ($sess_f=mysql_fetch_assoc($sess_q))
													{
									...
									{
										$topic_s="select topic_id,gi_num,gi_den,pi from lesson_plan where class_id='$class_id'";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$i=0;
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$stud_s="select student_id,student_gi_num,student_gi_den,student_pi from student_class_map where student_id='$student_id' and class_id='$class_id'";
										$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
										$stud_f=mysql_fetch_assoc($stud_q);
										if($index_type==1)
										{
											$num=$stud_f['student_gi_num'];
											$den=$stud_f['student_gi_den'];
											if($den==0)
											{
												$index_v=0;
											}
									...
									{
										$stud_s="select student_id,student_gi_num,student_gi_den,student_pi from student_class_map where class_id='$class_id'";
										$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
										if($index_type==1)
										{
											$i=0;
											while ($stud_f=mysql_fetch_assoc($stud_q))
											{
												$stud_v=$stud_f['student_id'];
												$num=$stud_f['student_gi_num'];
												$den=$stud_f['student_gi_den'];
									...
									{
										$class_s="select round(((gi_num*100)/gi_den),2) as grasp_index,pi from classes where class_id='$class_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										$class_f=mysql_fetch_assoc($class_q);
										if($index_type==1)
										{
											$index_v=$class_f['grasp_index'];
											if($index_v==null)
											{
												$index_v=0;
											}
						...
						//class_id has aggregation_level=0
						elseif (!strcasecmp($class_id, 'Agg_Lev=0'))
						{
							...
							if(strcasecmp($session_id, 'Agg_Lev=0') && strcasecmp($session_id, 'Agg_Lev=1'))
							{
								...
								if(strcasecmp($topic_id, 'Agg_Lev=0') && strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$stud_s="select round(((student_gi_num*100)/student_gi_den),2) as grasp_index,student_pi from stud_topic_time where student_id='$student_id' and topic_id='$topic_id' and class_session_id='$session_id'";
										$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
										$stud_f=mysql_fetch_assoc($stud_q);
										if($index_type==1)
										{
											$index_v=$stud_f['grasp_index'];
											if($index_v==null)
											{
												$index_v=0;
											}
									...
									{
										$stud_s="select student_id,student_gi_num,student_gi_den,student_pi from stud_topic_time where class_session_id='$session_id' and topic_id='$topic_id'";
										$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$stud_s="select student_gi_num,student_gi_den,student_pi from stud_topic_time where class_session_id='$session_id' and topic_id='$topic_id'";
										$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
										if($index_type==1)
										{
											$num=0;
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=0'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$topic_s="select topic_id,student_gi_num,student_gi_den,student_pi from stud_topic_time where student_id='$student_id' and class_session_id='$session_id'";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$i=0;
											...
											{
												$topic_v=$topic_f['topic_id'];
												$num=$topic_f['student_gi_num'];
												$den=$topic_f['student_gi_den'];
												if($den==0)
												{
													$index_v=0;
												}
									...
									{
										$topic_s="select distinct topic_id from stud_topic_time where class_session_id='$session_id'";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$i=0;
											...
											{
												$topic_v=$topic_f['topic_id'];
												$stud_s="select student_id,student_gi_num,student_gi_den from stud_topic_time where topic_id='$topic_v' and class_session_id='$session_id'";
												$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
												while ($stud_f=mysql_fetch_array($stud_q))
												{
													$stud_v=$stud_f['student_id'];
										...
										else if($index_type==2)
										{
											...
											{
												$topic_v=$topic_f['topic_id'];
												$stud_s="select student_id,student_pi from stud_topic_time where topic_id='$topic_v' and class_session_id='$session_id'";
												$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
												while ($stud_f=mysql_fetch_array($stud_q))
												{
													$stud_v=$stud_f['student_id'];
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$topic_s="select student_gi_num,student_gi_den,student_pi from stud_topic_time where student_id='$student_id' and class_session_id='$session_id'";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$num=0;
									...
									{
										$stud_s="select student_id,student_gi_num,student_gi_den,student_pi from stud_session_time where class_session_id='$session_id'";
										$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$sess_s="select gi_num,gi_den,pi from class_sessions where class_session_id='$session_id'";
										$sess_q=SunDataBaseManager::getSingleton()->QueryDB($sess_s);
										$sess_f=mysql_fetch_assoc($sess_q);
										if($index_type==1)
										{
											$num=$sess_f['gi_num'];
											$den=$sess_f['gi_den'];
							...
							else if (!strcasecmp($session_id, 'Agg_Lev=0'))
							{
								...
								if(strcasecmp($topic_id, 'Agg_Lev=0') && strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$sess_s="select stud_topic_time.class_session_id,class_id,student_gi_num,student_gi_den,student_pi from stud_topic_time inner join class_sessions on class_sessions.class_session_id=stud_topic_time.class_session_id where topic_id='$topic_id' and student_id='$student_id'";
										$sess_q=SunDataBaseManager::getSingleton()->QueryDB($sess_s);
										if($index_type==1)
										{
											$i=0;
											while ($sess_f=mysql_fetch_assoc($sess_q))
											{
												...
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$CLASSID]=$class_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$SESSIONID]=$sess_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$GRASPINDEX]=$index_v;
												$i++;
											}
										}
										else if($index_type==2)
										{
											...
											while ($sess_f=mysql_fetch_assoc($sess_q))
											{
												...
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$CLASSID]=$class_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$SESSIONID]=$sess_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$PARTICIPATIONINDEX]=$index_v;
												$i++;
											}
										}
									}
									...
									{
										$class_s="select lesson_plan.class_id,stud_topic_time.class_session_id,stud_topic_time.student_id,student_gi_num,student_gi_den,student_pi from lesson_plan inner join class_sessions on class_sessions.class_id=lesson_plan.class_id inner join stud_topic_time on stud_topic_time.class_session_id=class_sessions.class_session_id where lesson_plan.topic_id='$topic_id' and stud_topic_time.topic_id='$topic_id' order by class_id desc,class_session_id desc,student_id asc";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
											while ($class_f=mysql_fetch_array($class_q))
											{
												$class_v=$class_f['class_id'];
												$sess_v=$class_f['class_session_id'];
												$stud_v=$class_f['student_id'];
												$num=$class_f['student_gi_num'];
												$den=$class_f['student_gi_den'];
												if($den==0)
												{
													$index_v=0;
												}
												...
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$CLASSID]=$class_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$SESSIONID]=$sess_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$STUDENTID]=$stud_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$GRASPINDEX]=$index_v;
												$i++;
											}
										}
										else if($index_type==2)
										{
											...
											while ($class_f=mysql_fetch_array($class_q))
											{
												...
												$sess_v=$class_f['class_session_id'];
												$stud_v=$class_f['student_id'];
												$pi=$class_f['student_pi'];
												$index_v=substr((($num*100)/$den),0,strpos((($num*100)/$den),'.')+3);
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$CLASSID]=$class_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$SESSIONID]=$sess_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$STUDENTID]=$stud_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$PARTICIPATIONINDEX]=$pi;
												$i++;
											}
										}
									}
									...
									{
										$class_s="select distinct class_sessions.class_session_id,class_sessions.class_id,class_sessions.gi_num,class_sessions.gi_den,class_sessions.pi from class_sessions inner join lesson_plan on lesson_plan.class_id=class_sessions.class_id inner join stud_topic_time on stud_topic_time.class_session_id=class_sessions.class_session_id where lesson_plan.topic_id='$topic_id' and stud_topic_time.topic_id='$topic_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=0'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$class_s="select class_sessions.class_id,stud_topic_time.class_session_id,stud_topic_time.topic_id,student_gi_num,student_gi_den,student_pi from stud_topic_time inner join class_sessions on class_sessions.class_session_id=stud_topic_time.class_session_id where student_id='$student_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
											while ($class_f=mysql_fetch_array($class_q))
											{
												...
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$CLASSID]=$class_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$SESSIONID]=$sess_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$TOPICID]=$topic_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$CLASS][$i][SMC::$GRASPINDEX]=$index_v;
												$i++;
											}
										}
									...
									{
										$class_s="select class_sessions.class_id,stud_topic_time.class_session_id,topic_id,student_id,student_gi_num,student_gi_den,student_pi from stud_topic_time inner join class_sessions on stud_topic_time.class_session_id=class_sessions.class_session_id inner join classes on classes.class_id=class_sessions.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$class_s="select distinct stud_topic_time.topic_id,class_sessions.class_id,stud_topic_time.class_session_id,school_id,sum(student_gi_num) as tpc_gi_num,sum(student_gi_den) as tpc_gi_den,sum(student_pi) as tpc_pi from stud_topic_time inner join class_sessions on class_sessions.class_session_id=stud_topic_time.class_session_id inner join classes on class_sessions.class_id=classes.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id' group by topic_id,stud_topic_time.topic_id,stud_topic_time.class_session_id";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
											while ($class_f=mysql_fetch_array($class_q))
											{
												...
												$sess_v=$class_f['class_session_id'];
												$topic_v=$class_f['topic_id'];
												$num=$class_f['tpc_gi_num'];
												$den=$class_f['tpc_gi_den'];
												if($den==0)
												{
													$index_v=0;
												}
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$class_s="select stud_session_time.class_session_id,class_sessions.class_id,student_gi_num,student_gi_den,student_pi from stud_session_time inner join class_sessions on class_sessions.class_session_id=stud_session_time.class_session_id where student_id='$student_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
 											while ($class_f=mysql_fetch_assoc($class_q))
 											{
 												$class_v=$class_f['class_id'];
 												$sess_v=$class_f['class_session_id'];
 												$num=$class_f['student_gi_num'];
									...
									{
										$class_s="select class_sessions.class_id,stud_session_time.class_session_id,student_id,student_gi_num,student_gi_den,student_pi from stud_session_time inner join class_sessions on stud_session_time.class_session_id=class_sessions.class_session_id inner join classes on classes.class_id=class_sessions.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$class_s="select class_session_id,class_sessions.class_id,class_sessions.gi_num,class_sessions.gi_den,class_sessions.pi from class_sessions inner join classes on class_sessions.class_id=classes.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id' ORDER BY class_sessions.class_session_id DESC";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
							...
							else if (!strcasecmp($session_id, 'Agg_Lev=1'))
							{
								...
								if(strcasecmp($topic_id, 'Agg_Lev=0') && strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$class_s="select class_id,sum(student_gi_num) as student_gi_num,sum(student_gi_den) as student_gi_den,sum(student_pi) as student_pi from stud_topic_time inner join class_sessions on class_sessions.class_session_id=stud_topic_time.class_session_id where topic_id='$topic_id' and student_id='$student_id' group by class_sessions.class_id";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$class_s="select student_id,class_sessions.class_id,sum(student_gi_num) as stud_gi_num,sum(student_gi_den) as stud_gi_den,sum(student_pi) as stud_pi from stud_topic_time inner join class_sessions on stud_topic_time.class_session_id=class_sessions.class_session_id where topic_id='$topic_id' group by student_id,class_id ORDER BY student_id,class_id ASC";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
											while ($class_f=mysql_fetch_array($class_q))
											{
												$class_v=$class_f['class_id'];
												$stud_v=$class_f['student_id'];
												$num=$class_f['stud_gi_num'];
												$den=$class_f['stud_gi_den'];
												if($den==0)
												{
													$index_v=0;
												}
									...
									{
										$class_s="select class_id,gi_num,gi_den,pi from lesson_plan where topic_id='$topic_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=0'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$class_s="select topic_id,class_sessions.class_id,sum(student_gi_num) as stud_gi_num,sum(student_gi_den) as stud_gi_den,sum(student_pi) as stud_pi from stud_topic_time inner join class_sessions on stud_topic_time.class_session_id=class_sessions.class_session_id where student_id='$student_id' group by topic_id,class_id ORDER BY topic_id,class_id ASC";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$class_s="select student_id,topic_id,class_sessions.class_id,sum(student_gi_num) as stud_gi_num,sum(student_gi_den) as stud_gi_den,sum(student_pi) as stud_pi from stud_topic_time inner join class_sessions on stud_topic_time.class_session_id=class_sessions.class_session_id inner join classes on classes.class_id=class_sessions.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id' group by student_id,topic_id,class_id ORDER BY student_id,class_id,topic_id ASC";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$class_s="select topic_id,lesson_plan.class_id,lesson_plan.gi_num,lesson_plan.gi_den,lesson_plan.pi from lesson_plan inner join classes on classes.class_id=lesson_plan.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$class_s="select class_id,max(gi_class_num) as gi_class_num,max(gi_class_den) as gi_class_den,max(pi_class) as pi_class from stud_session_time as sstime inner join class_sessions as csess on csess.class_session_id=sstime.class_session_id where student_id='$student_id' group by class_id order by class_id asc";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
											...
											{
												$class_v=$class_f['class_id'];
												$num=$class_f['gi_class_num'];
												$den=$class_f['gi_class_den'];
												if($den==0)
												{
													$index_v=0;
												}
									...
									{
										$class_s="select student_id,csess.class_id,max(gi_class_num) as gi_class_num,max(gi_class_den) as gi_class_den,max(pi_class) as pi_class from stud_session_time as sstime inner join class_sessions as csess on csess.class_session_id=sstime.class_session_id inner join classes on classes.class_id=csess.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id' group by csess.class_id,student_id order by csess.class_id asc";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$class_s="select class_id,gi_num,gi_den,pi from classes inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
						...
						//class_id has aggregation_level=1
						elseif (!strcasecmp($class_id, 'Agg_Lev=1'))
						{
							...
							if(strcasecmp($session_id, 'Agg_Lev=0') && strcasecmp($session_id, 'Agg_Lev=1'))
							{
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=0'))
								{
									...
									{
										$topic_s="select student_id,topic_id,student_gi_num,student_gi_den,student_pi from stud_topic_time where class_session_id='$session_id' order by topic_id asc";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$topic_s="select topic_id,sum(student_gi_num) as student_gi_num,sum(student_gi_den) as student_gi_den,sum(student_pi) as student_pi from stud_topic_time where class_session_id=2430 group by topic_id";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$i=0;
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$stud_s="select student_gi_num,student_gi_den,student_pi from stud_session_time where class_session_id='$session_id' and student_id='$student_id'";
										$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
										$stud_f=mysql_fetch_assoc($stud_q);
										if($index_type==1)
										{
							...
							else if (!strcasecmp($session_id, 'Agg_Lev=0'))
							{
								...
								if(strcasecmp($topic_id, 'Agg_Lev=0') && strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$sess_s="select stud_topic_time.class_session_id,student_gi_num,student_gi_den,student_pi from stud_topic_time where topic_id='$topic_id' and student_id='$student_id'";
										$sess_q=SunDataBaseManager::getSingleton()->QueryDB($sess_s);
										if($index_type==1)
										{
											$i=0;
											while ($sess_f=mysql_fetch_assoc($sess_q))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$sess_v;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v;
												$i++;
											}
										}
										else if($index_type==2)
										{
											...
											while ($sess_f=mysql_fetch_assoc($sess_q))
											{
												...
												$index_v=$pi;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$sess_v;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$PARTICIPATIONINDEX]=$index_v;
												$i++;
											}
										}
									}
									...
									{
										$class_s="select stud_topic_time.class_session_id,stud_topic_time.student_id,student_gi_num,student_gi_den,student_pi from lesson_plan inner join class_sessions on class_sessions.class_id=lesson_plan.class_id inner join stud_topic_time on stud_topic_time.class_session_id=class_sessions.class_session_id where lesson_plan.topic_id='$topic_id' and stud_topic_time.topic_id='$topic_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
											while ($class_f=mysql_fetch_array($class_q))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$sess_v;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$STUDENTID]=$stud_v;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v;
												$i++;
											}
										}
									...
									{
										$sess_s="select stud_topic_time.class_session_id,sum(student_gi_num) as student_gi_num,sum(student_gi_den) as student_gi_den,sum(student_pi) as student_pi from stud_topic_time where topic_id='$topic_id' group by stud_topic_time.class_session_id";
										$sess_q=SunDataBaseManager::getSingleton()->QueryDB($sess_s);
										if($index_type==1)
										{
											$i=0;
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=0'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$class_s="select stud_topic_time.class_session_id,stud_topic_time.topic_id,student_gi_num,student_gi_den,student_pi from stud_topic_time where student_id='$student_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
											while ($class_f=mysql_fetch_array($class_q))
											{
												...
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$SESSIONID]=$sess_v;
												$arr[SMC::$SESSIONCONTAINER][SMC::$SESSION][$i][SMC::$TOPICID]=$topic_v;
												$arr[SMC::$CLASSCONTAINER][SMC::$SESSION][$i][SMC::$GRASPINDEX]=$index_v;
												$i++;
											}
										}
										else if($index_type==2)
									...
									{
										$class_s="select stud_topic_time.class_session_id,topic_id,student_id,student_gi_num,student_gi_den,student_pi from stud_topic_time inner join class_sessions on stud_topic_time.class_session_id=class_sessions.class_session_id inner join classes on classes.class_id=class_sessions.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$class_s="select distinct stud_topic_time.topic_id,stud_topic_time.class_session_id,school_id,sum(student_gi_num) as tpc_gi_num,sum(student_gi_den) as tpc_gi_den,sum(student_pi) as tpc_pi from stud_topic_time inner join class_sessions on class_sessions.class_session_id=stud_topic_time.class_session_id inner join classes on class_sessions.class_id=classes.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id' group by topic_id";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$class_s="select stud_session_time.class_session_id,student_gi_num,student_gi_den,student_pi from stud_session_time inner join class_sessions on class_sessions.class_session_id=stud_session_time.class_session_id where student_id='$student_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$class_s="select stud_session_time.class_session_id,student_id,student_gi_num,student_gi_den,student_pi from stud_session_time inner join class_sessions on stud_session_time.class_session_id=class_sessions.class_session_id inner join classes on classes.class_id=class_sessions.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$sess_s="select class_session_id,class_sessions.gi_num,class_sessions.gi_den,class_sessions.pi from class_sessions inner join classes on class_sessions.class_id=classes.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id' order by class_session_id desc";
										$sess_s=SunDataBaseManager::getSingleton()->QueryDB($sess_s);
										if($index_type==1)
										{
											$i=0;
											while ($sess_f=mysql_fetch_array($sess_s))
											{
												$sess_v=$sess_f['class_session_id'];
												$num=$class_f['gi_num'];
 												$den=$class_f['gi_den'];
							...
							else if (!strcasecmp($session_id, 'Agg_Lev=1'))
							{
								...
								if(strcasecmp($topic_id, 'Agg_Lev=0') && strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$class_s="select sum(stud_gi_num) as stud_gi_num, sum(stud_gi_den) as stud_gi_den, sum(stud_pi) as stud_pi from (select sum(student_gi_num) as stud_gi_num,sum(student_gi_den) as stud_gi_den,sum(student_pi) as stud_pi from stud_topic_time inner join class_sessions on stud_topic_time.class_session_id=class_sessions.class_session_id inner join classes on classes.class_id=class_sessions.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where student_id='$student_id' and topic_id='$topic_id' and school_id='$school_id' group by class_sessions.class_id) class_agg";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										$class_f=mysql_fetch_array($class_q);
										if($index_type==1)
										{
											$num=$class_f['stud_gi_num'];
 											$den=$class_f['stud_gi_den'];
									...
									{
										$stud_s="select student_id,sum(student_gi_num) as stud_gi_num,sum(student_gi_den) as stud_gi_den,sum(student_pi) as stud_pi from stud_topic_time inner join class_sessions on stud_topic_time.class_session_id=class_sessions.class_session_id inner join classes on classes.class_id=class_sessions.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where topic_id='$topic_id' and school_id='$school_id' group by student_id";
										$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
										if($index_type==1)
										{
											$i=0;
											...
											{
												$stud_v=$stud_f['student_id'];
												$num=$stud_f['stud_gi_num'];
 												$den=$stud_f['stud_gi_den'];
 												if($den==0)
												{
													$index_v=0;
												}
									...
									{
										$class_s="select sum(gi_num), sum(gi_den), sum(pi) from lesson_plan where topic_id='$topic_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										$class_f=mysql_fetch_array($class_q);
										if($index_type==1)
										{
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=0'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$topic_s="select topic_id,sum(student_gi_num) as student_gi_num,sum(student_gi_den) as student_gi_den,sum(student_pi) as student_pi from stud_topic_time inner join class_sessions on class_sessions.class_session_id=stud_topic_time.class_session_id where student_id='$student_id' group by student_id,topic_id order by topic_id desc";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$topic_s="select student_id,topic_id,sum(student_gi_num) as student_gi_num,sum(student_gi_den) as student_gi_den,sum(student_pi) as student_pi from stud_topic_time inner join class_sessions on class_sessions.class_session_id=stud_topic_time.class_session_id inner join classes on classes.class_id=class_sessions.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id' group by student_id,topic_id order by topic_id desc,student_id asc";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$i=0;
									...
									{
										$topic_s="select topic_id,sum(lesson_plan.gi_num) as gi_num,sum(lesson_plan.gi_den) as gi_den, sum(lesson_plan.pi) as pi from lesson_plan inner join classes on classes.class_id=lesson_plan.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id' group by topic_id order by topic_id desc";
										$topic_q=SunDataBaseManager::getSingleton()->QueryDB($topic_s);
										if($index_type==1)
										{
											$i=0;
								...
								else if (!strcasecmp($topic_id, 'Agg_Lev=1'))
								{
									...
									if(strcasecmp($student_id, 'Agg_Lev=0') && strcasecmp($student_id, 'Agg_Lev=1'))
									{
										$class_s="SELECT sum(gi_class_num) as gi_class_num,sum(gi_class_den) as gi_class_den,sum(pi_class) as pi_class from (select class_id,max(gi_class_num) as gi_class_num,max(gi_class_den) as gi_class_den,max(pi_class) as pi_class from stud_session_time inner join class_sessions on class_sessions.class_session_id=stud_session_time.class_session_id where student_id='$student_id' group by class_id) class_agg";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										$class_f=mysql_fetch_array($class_q);
										if($index_type==1)
										{
									...
									{
										$stud_s="select student_id,sum(class_agg.gi_class_num) as gi_class_num,sum(class_agg.gi_class_den) as gi_class_den,sum(class_agg.pi_class) as pi_class from (SELECT student_id,class_sessions.class_id,max(gi_class_num) as gi_class_num,max(gi_class_den) as gi_class_den,max(pi_class) as pi_class from stud_session_time inner join class_sessions on class_sessions.class_session_id=stud_session_time.class_session_id inner join classes on classes.class_id=class_sessions.class_id inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id=3 group by class_id,student_id order by class_id desc,student_id asc) class_agg group by class_agg.student_id order by class_agg.student_id asc";
										$stud_q=SunDataBaseManager::getSingleton()->QueryDB($stud_s);
										if($index_type==1)
										{
											$i=0;
											...
											{
												$stud_v=$stud_f['student_id'];
												$num=$stud_f['gi_class_num'];
 												$den=$stud_f['gi_class_den'];
 												if($den==0)
												{
													$index_v=0;
												}
									...
									{
										$class_s="select sum(gi_num) as gi_num,sum(gi_den) as gi_den,sum(pi) as pi from classes inner join academic_terms on academic_terms.academic_term_id=classes.academic_term_id where school_id='$school_id'";
										$class_q=SunDataBaseManager::getSingleton()->QueryDB($class_s);
										$class_f=mysql_fetch_array($class_q);
										if($index_type==1)
										{
				...
				public function CreateCategory($category_title=null,$topic_id=null,$category_description=null,$user_id=null,$uuid=null)
				{
					try
					{
						$current_time_select="select current_timestamp";
				...
				public function SaveSuggestionState($suggestion_id=null,$suggestion_state=null,$user_id=null,$uuid=null)
				{
					try
					{
						...
						if(is_array($suggestion_ids) && is_array($suggestion_states))
						{
							...
							while ($i<((count($suggestion_ids)<count($suggestion_states)) ? count($suggestion_ids) : count($suggestion_states)))
							{
								...
								if($sugg_state==29)
								{
									...
									$arr[SMC::$SUGGESTIONCONTAINER][SMC::$SUGGESTIONARRAY][$i][SMC::$SUGGESTIONID]=$sugg_id;
									$arr[SMC::$SUGGESTIONCONTAINER][SMC::$SUGGESTIONARRAY][$i][SMC::$SUGGESTTXT]=$suggestion_txt;
									$arr[SMC::$SUGGESTIONCONTAINER][SMC::$SUGGESTIONARRAY][$i][SMC::$ELEMENTID]=$rec_elem;
									$arr[SMC::$SUGGESTIONCONTAINER][SMC::$SUGGESTIONARRAY][$i][SMC::$ELEMENTFLAG]=1;
								}
								if($old_state_id==29 && $sugg_state==28)
								{
									...
									$arr[SMC::$SUGGESTIONCONTAINER][SMC::$SUGGESTIONARRAY][$i][SMC::$SUGGESTIONID]=$sugg_id;
									$arr[SMC::$SUGGESTIONCONTAINER][SMC::$SUGGESTIONARRAY][$i][SMC::$SUGGESTTXT]=$suggestion_txt;
									$arr[SMC::$SUGGESTIONCONTAINER][SMC::$SUGGESTIONARRAY][$i][SMC::$ELEMENTID]=$sel_elem_v;
									$arr[SMC::$SUGGESTIONCONTAINER][SMC::$SUGGESTIONARRAY][$i][SMC::$ELEMENTFLAG]=0;
								}
						...
						else if(!(is_array($suggestion_ids)) || !(is_array($suggestion_states)))
						{
							...
							if(!(is_array($suggestion_ids)) && is_array($suggestion_states))
							{
								...
								if($sugg_state1==29)
								{
									...
									$arr[SMC::$SUGGESTTXT]=$suggestion_txt;
				...
				public function SelectCategory($category_id=null,$user_id=null,$uuid=null)
				{
					try
					{
						$arr[SMC::$STATUS]="Success";
						...
						while ($row=mysql_fetch_array($elems_q))
						{
							...
							$arr[SMC::$ELEMENTCONTAINER][SMC::$ELEMENT][$i][SMC::$ELEMENTID]=$elem_id;
            				$arr[SMC::$ELEMENTCONTAINER][SMC::$ELEMENT][$i][SMC::$ELEMENTTEXT]=$elem_text;
            				$arr[SMC::$ELEMENTCONTAINER][SMC::$ELEMENT][$i][SMC::$SUGGESTIONID]=$sugg_id;
							$i++;
						}
						$retval = $this->mXMLManager->createXML($arr);
						return $retval;
				...
				public function UpdateBehaviourIndex($session_id=null,$student_id=null,$transaction_id=null,$user_id=null,$uuid=null)
				{
					try
					{
						$arr[SMC::$STATUS]="Success";
				...
				public function GetLiveStatus($session_id=null,$user_id=null,$uuid=null)
				{
					try
					{
						$sel_questions="SELECT current_topic,current_question,parent_topic_id,starts_on
						FROM live_session_status
						INNER JOIN topic
						on topic.topic_id=live_session_status.current_topic
                        INNER JOIN class_sessions
                        on live_session_status.session_id=class_sessions.class_session_id
						where session_id='$session_id'";
						$questions_q=SunDataBaseManager::getSingleton()->QueryDB($sel_questions);
						$live=SunDataBaseManager::getSingleton()->getnoOfrows($questions_q);
						if($live==0)
						{
							$arr[SMC::$STATUS]="No live session";
						}
						else if($questions_q)
						{
							$questions_f=mysql_fetch_assoc($questions_q);
							$question_id=$questions_f['current_question'];
							$topic_id=$questions_f['current_topic'];
							$parent_topic_id=$questions_f['parent_topic_id'];
							$started_at=$questions_f['starts_on'];
							$arr[SMC::$STATUS]="Success";
							$arr[SMC::$QUESTIONID]=$question_id;
							$arr[SMC::$TOPICID]=$topic_id;
							$arr[SMC::$PARENTTOPICID]=$parent_topic_id;
							$arr[SMC::$STARTEDAT]=$started_at;
						}
						else 
						{
							$arr[SMC::$STATUS]="Failure";
				...
				public function GetLikertScales($user_id=null,$uuid=null)
				{
					try
					{
						$sel_like_rt="select * from likert_scales";
						$likert_q=SunDataBaseManager::getSingleton()->QueryDB($sel_like_rt);
						if($likert_q)
						{
							$arr[SMC::$STATUS]="Success";
						}
						...
						$i=0;
						while ($likert_f=mysql_fetch_array($likert_q))
						{
							$name=$likert_f['name'];
							$op1=$likert_f['op1'];
							$op2=$likert_f['op2'];
							...
							$op6=$likert_f['op6'];
							$op7=$likert_f['op7'];
							$arr[SMC::$OPCONTAINER][SMC::$OP][$i][SMC::$NAME]=$name;
							$arr[SMC::$OPCONTAINER][SMC::$OP][$i][SMC::$OP1]=$op1;
							$arr[SMC::$OPCONTAINER][SMC::$OP][$i][SMC::$OP2]=$op2;
							$arr[SMC::$OPCONTAINER][SMC::$OP][$i][SMC::$OP3]=$op3;
							$arr[SMC::$OPCONTAINER][SMC::$OP][$i][SMC::$OP4]=$op4;
							$arr[SMC::$OPCONTAINER][SMC::$OP][$i][SMC::$OP5]=$op5;
							$arr[SMC::$OPCONTAINER][SMC::$OP][$i][SMC::$OP6]=$op6;
							$arr[SMC::$OPCONTAINER][SMC::$OP][$i][SMC::$OP7]=$op7;
							$i++;
						}
						$retval = $this->mXMLManager->createXML($arr);
						return $retval;
				...
				public function GetErrorMessages($user_id=null,$uuid=null)
				{
					try
					{
						$sel_errors="select * from error_messages";
						$error_q=SunDataBaseManager::getSingleton()->QueryDB($sel_errors);
						if($error_q)
						{
							$arr[SMC::$STATUS]="Success";
						}
						...
						$i=0;
						while ($error_f=mysql_fetch_array($error_q))
						{
							$error_id=$error_f['error_id'];
							$error_code=$error_f['error_code'];
							$error_msg=$error_f['error_message'];
							$arr[SMC::$ERRORCONTAINER][SMC::$ERROR][$i][SMC::$ERRORID]=$error_id;
							$arr[SMC::$ERRORCONTAINER][SMC::$ERROR][$i][SMC::$ERRORCODE]=$error_code;
							$arr[SMC::$ERRORCONTAINER][SMC::$ERROR][$i][SMC::$ERRORMESSAGE]=$error_msg;
							$i++;
						}
						$retval = $this->mXMLManager->createXML($arr);
						return $retval;
