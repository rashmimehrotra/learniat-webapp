	...
	var Easycal = {
		...
		_getMinorSlotForCell : function(momTime){
			...
			if(this.options.timeGranularity < this.options.slotDuration){
				var middleDivSlot = this.options.slotDuration/2;
				for(var i = 0 ; i < (this.options.slotDuration/this.options.timeGranularity); i++ ){
					var styleDiv = ''
					if (middleDivSlot == i) {
						styleDiv = "style='border-bottom: 1px dashed #E5E5E5;'";
					}
					html += '<div class="' + classes.minorSlot + '" data-time="' + time.format(format.timeLong) + '" ' + styleDiv +'></div>';
					time.add(this.options.timeGranularity,'m');
				}
			}
			return html;
